...
struct ObjectModel
{
    ...
    inline static constexpr auto id_columns =
        orm::primaryKey<&ObjectModel::field1, &ObjectModel::field2>();
...
struct User
{
    ...
    // The id column is omitted from INSERT and generated by the database.
...
`manyToMany<&User::roles>().through<"user_roles">()`; inverse mappings can use
typed member pointers such as `mappedBy<&User::roles>()`. Relation tables are
managed with `createRelationTables<T>()` and `deleteRelationTables<T>()`, while
individual links are changed with `link` and `unlink`. In-memory collection
changes never cascade-save endpoints or synchronize the database.
